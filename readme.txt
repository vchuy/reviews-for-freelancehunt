=== Reviews for freelancehunt ===
Contributors: v.chuy
Donate link: https://www.vchuy-develop.com/donat/
Tags: Reviews for freelancehunt, multilingual, feedback, freelancehunt.com 
Requires at least: 2.3
Tested up to: 5.3.2
Requires PHP: 5.6
Stable tag: 2.00
License: GPLv2 or later
License URI: https://www.gnu.org/licenses/gpl-2.0.html

Plugin to display reviews from the freelancehunt.com website on your website page.

== Description ==

Plugin to display reviews from the freelancehunt.com website on your website page.

Add compatible with api 2.0



= Field types include: =

* input fields


== Installation ==


= Uploading in WordPress Dashboard =

1. Navigate to the 'Add New' in the plugins dashboard
2. Navigate to the 'Upload' area
3. Select `freelancehunt-revievs.zip` from your computer
4. Click 'Install Now'
5. Activate the plugin in the Plugin dashboard


= Using FTP =

1. Upload `freelancehunt-revievs` folder to the `/wp-content/plugins/` directory.
2. Activate the plugin through the 'Plugins' menu in WordPress.
3. Go to Settings -> Freelancehunt reviews
4. Fill fields the ID and secret key
5. Use shortcode [freelance] on page

== Frequently Asked Questions ==

Do you have questions or issues with Reviews for freelancehunt? Use these support channels appropriately.

1. [Docs](https://www.vchuy-develop.com/otzyvy-o-frilansere-s-freelancehunt-com/)

[Support](https://www.vchuy-develop.com/en/)

== Screenshots ==

1. screenshot-1.png


== Changelog ==

= 1.00 =
* First version

= 2.00 =
* Compatible with api 2.0


== Upgrade Notice ==

= 1.0 =
Initial release

= 2.00 =
* Compatible with api 2.0

